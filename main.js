...
class ClientAPI {
  ...
  getCookieData(setCookie) {
    try {
      ...
      this.log(`Error getting cookie: ${error.message}`, "error");
      return null;
    }
  }
    ...
    if (resCreate.success && resCreate.data?.quiz_id) {
      ...
      if (resGet.success) {
        ...
        for (const q of quests) {
          await sleep(1);
          this.log(`Completing quest: ${q.content}`);
          const payload = {
  ...
  async handleOnBoard() {
    ...
    for (const q of ques) {
      ...
      this.log(`Completing quiz ${content}`);
      const payload = {
  ...
  async handleMess(userData) {
    ...
    while (limit > 0) {
      ...
      this.log(`[${limit}/${total}] Sending message: ${mess}`);
...
async function main() {
  ...
  if (!resCheck.endpoint) return console.log(`Cannot find API ID, possibly a connection error, try again later!`.red);
